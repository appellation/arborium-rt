...
- **`@discord/arborium-rt`** — the TypeScript runtime wrapper. Ships
  ...
  `package`/`package-all`; themes and wasms are staged by `stage`.
...
# runs `stage` (themes + host + runtime wasms) and `package-all --only json`
...
- `packages/arborium-rt/dist/themes.js` — single `THEMES` aggregator.
  ...
  `stage` from the codegen binary's metadata output. Exposed as
