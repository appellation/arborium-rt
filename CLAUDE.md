...
- **`@discord/arborium-rt-wasm`** — the TypeScript runtime wrapper. Ships
  ...
  under `packages/arborium-rt-wasm/dist/grammars/<lang>/`, and a single
...
- **Cross-platform (standard napi model).** `package.json`'s
  ...
  linux x64/arm64 — all gnu — and win32 x64/arm64 msvc), and `napi create-npm-dirs` generated one
  ...
  `process.platform`/`arch`. The main package depends on the six sub-packages
  ...
  **workspace members** (see `pnpm-workspace.yaml`). That is the crux: pnpm
  ...
  the real version and publishes the main package + the six sub-packages.
...
structurally identical to `packages/arborium-rt-wasm/src` (`ThemedSpan`,
...
### TypeScript consumer package (`packages/arborium-rt-wasm/`)

Typed `Runtime` / `Grammar` / `Session` API over the ABI, published as
`@discord/arborium-rt-wasm`. `loadArboriumRuntime()` takes no args — the
host `.mjs`/wasm and the runtime wasm are staged into
`packages/arborium-rt-wasm/dist/` (under `host/` and `runtime/`) by
...
so consumers only need `import { GRAMMARS } from '@discord/arborium-rt-wasm'`.
...
- `packages/arborium-rt-wasm/dist/grammars.js` — single `GRAMMARS` aggregator.
  ...
  consumers as `import { GRAMMARS } from '@discord/arborium-rt-wasm'`.
...
- **Adding a new `arborium_rt_*` export requires three coordinated edits**:
  ...
  `packages/arborium-rt-wasm/src/abi.ts` (plus a method on the wrapper class if
