...
export async function loadArboriumRuntime(): Promise<Runtime> {
	...
	const host = await factory({
		wasmBinary: hostWasm as unknown as ArrayBuffer,
	});
...
export class Runtime {
	...
	async loadGrammar(options: LoadGrammarOptions): Promise<Grammar> {
		...
		const languageFn = pickLanguageExport(
			grammarExports,
			options.languageExport,
		);
...
export class Session {
	...
	parse(): Utf16ParseResult {
		...
		return this.#withJsonOutput(
			"parse-failed",
			(outPtr, outLen) =>
				this.grammar.runtime.abi.arborium_rt_parse_utf16(
					this.id,
					outPtr,
					outLen,
				),
...
async function loadBundledHostModuleFactory(): Promise<HostModuleFactory> {
	...
	const mod = (await import("./host/web-tree-sitter.mjs")) as {
		default: HostModuleFactory;
	};
	return mod.default;
}
...
function pickLanguageExport(
	...
	// biome-ignore lint/style/noNonNullAssertion: candidates[0] is defined (we just checked length === 1).
